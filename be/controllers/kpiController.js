...
class KpiController {
  ...
  static async getAllHpmKpi(req, res) {
    try {
      ...
      let query = `
        SELECT hpm_id, name, total_requests_updated, total_requests_done, avg_completion_time_hhmm, period
        FROM homepass_moving_address_hpm_kpi
        WHERE 1=1
      `;
      ...
      if (sortBy && ['name', 'total_requests_updated', 'total_requests_done', 'avg_completion_time_hhmm', 'period'].includes(sortBy)) {
        query += ` ORDER BY ${sortBy} ${sortOrder === 'DESC' ? 'DESC' : 'ASC'}`;
      } else {
        query += ` ORDER BY period DESC, total_requests_done DESC`;
      }
      ...
      const countQuery = `
        SELECT COUNT(*) FROM homepass_moving_address_hpm_kpi
        WHERE 1=1
        ${name ? ` AND name ILIKE $1` : ''}
        ${period ? ` AND period = $${name ? '2' : '1'}` : ''}
      `;
      ...
      console.error('Error in getAllHpmKpi:', error);
      res.status(500).json({ error: 'Internal server error' });
    }
  }
}
